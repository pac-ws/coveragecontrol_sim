...
namespace CoverageControlSim {
  ...
  void CoverageControlSimCentralized::CreateCmdSubscribers() {
    ...
    for (int robot_id = 0; robot_id < parameters_.pNumRobots; ++robot_id) {
      std::string topic_name = namespaces_of_robots_[robot_id] + "/cmd_relative_pose";
      auto sub = this->create_subscription<geometry_msgs::msg::PoseStamped>(
          topic_name, rclcpp::QoS(buffer_size_),
          [this,
           robot_id](const geometry_msgs::msg::PoseStamped::ConstSharedPtr msg)
              -> void {
            Point2 robot_pos;
    ...
    for (int robot_id = 0; robot_id < parameters_.pNumRobots; ++robot_id) {
      std::string topic_name = namespaces_of_robots_[robot_id] + "/cmd_global_pose";
      auto sub = this->create_subscription<geometry_msgs::msg::PoseStamped>(
          topic_name, rclcpp::QoS(buffer_size_),
          [this,
           robot_id](const geometry_msgs::msg::PoseStamped::ConstSharedPtr msg)
      ...
      auto sub = this->create_subscription<geometry_msgs::msg::TwistStamped>(
          ...
          [this,
           robot_id](const geometry_msgs::msg::TwistStamped::ConstSharedPtr msg)
            ...
            if (res) {
              RCLCPP_ERROR(this->get_logger(), "Error stepping action for robot %d",
                          robot_id);
            }
          },
          cbg_cmd_vel_opts);
  ...
  void CoverageControlSimCentralized::CreateRobotMapPublishers() {
    for (int robot_id = 0; robot_id < parameters_.pNumRobots; ++robot_id) {
      ...
      auto robot_map_pub_timer_callback = [this, robot_id, robot_map_pub]() -> void {
  ...
  void CoverageControlSimCentralized::CreateNeigborsPosPublisher() {
    for (int robot_id = 0; robot_id < parameters_.pNumRobots; ++robot_id) {
      std::string topic_name = namespaces_of_robots_[robot_id] + "/neighbors_pose";
  ...
  void CoverageControlSimCentralized::CreateGlobalMapPublisher() {
    ...
    global_map_pub_ = this->create_publisher<std_msgs::msg::Float32MultiArray>(
        topic_name, 3);
  ...
  void CoverageControlSimCentralized::CreateSystemMapPublisher() {
    ...
    system_map_pub_ = this->create_publisher<std_msgs::msg::Float32MultiArray>(
        topic_name, 3);
    ...
    auto system_map_pub_timer_callback = [this, system_map_pub]() -> void {
      ...
      /* auto system_map_msg = EigenMatrixRowMajorToFloat32MultiArray(system_map(Eigen::seq(0, Eigen::last, 2), Eigen::seq(0, Eigen::last, 2))); */
  ...
  void CoverageControlSimCentralized::CreateExploredIDFMapPublisher() {
    ...
    global_explored_idf_map_pub_timer_ =
        this->create_wall_timer(100ms, global_explored_idf_map_pub_timer_callback);
  }
  ...
  void CoverageControlSimCentralized::CreateAllRobotsPosesPublisher() {
    ...
    auto robot_poses_pub_timer_callback = [this, robot_poses_pub]() -> void {
      ...
      for (int robot_id = 0; robot_id < parameters_.pNumRobots; ++robot_id) {
        ...
        transform.transform.rotation.x = 0.0; transform.transform.rotation.y = 0.0; transform.transform.rotation.z = 0.0; transform.transform.rotation.w = 1.0;
  ...
  void CoverageControlSimCentralized::CreateRobotLocalMapPublishers() {
    for (int robot_id = 0; robot_id < parameters_.pNumRobots; ++robot_id) {
      ...
      // Create timer to publish local map
      auto robot_local_map_pub_timer_callback =
          [this, robot_id, robot_local_map_pub]() -> void {
  ...
  void CoverageControlSimCentralized::CreateSensorViewPublisher() {
    for (int robot_id = 0; robot_id < parameters_.pNumRobots; ++robot_id) {
      ...
      auto robot_sensor_view_pub_timer_callback =
        ...
        MapType sensor_view =
            coverage_system_ptr_->GetRobotSensorView(robot_id);
  ...
  void CoverageControlSimCentralized::CreateCmdVelPublisher() {
    ...
    auto cmd_vel_pub_timer_callback = [this]() -> void {
      auto voronoi =
          Voronoi(sim_robot_positions_, coverage_system_ptr_->GetSystemExploredIDFMap(),
  ...
  /*        *1/ */
  /*       /1*             cmd_vel_msg.twist.linear.x, cmd_vel_msg.twist.linear.y); */
  /*        *1/ */
  /*     }; */
