...
class CoverageControlSim : public rclcpp::Node {
	...
	public:
		...
		CoverageControlSim() : Node("coveragecontrol_sim"), coverage_system_ptr_(nullptr) {
			...
			controller_ = CoverageAlgorithm(parameters_, parameters_.pNumRobots, *coverage_system_ptr_);
		...
		void CreateCmdVelPublisher() {
			for (int robot_id = 0; robot_id < parameters_.pNumRobots; robot_id++) {
				std::string topic_name = "robot_" + std::to_string(robot_id) + "/cmd_vel";
				cmd_vel_pubs_.push_back(this->create_publisher<geometry_msgs::msg::Twist>(topic_name, buffer_size_));
				auto cmd_vel_pub = cmd_vel_pubs_.back();
				// Create timer to publish robot position
				auto cmd_vel_pub_timer_callback = [this, robot_id, cmd_vel_pub]() -> void {
					controller_.Step();
					auto actions = controller_.GetActions();
					double velocity_x = actions[0];
					double velocity_y = actions[1];
					// Convert to x velocity and angular velocity
					geometry_msgs::msg::Twist cmd_vel_msg;
					double angle = atan2(velocity_y, velocity_x);
					cmd_vel_msg.linear.x = velocity_x * cos(angle);
					cmd_vel_msg.angular.z = angle;
					cmd_vel_pub->publish(cmd_vel_msg);
				};
				cmd_vel_pub_timers_.push_back(this->create_wall_timer(30ms, cmd_vel_pub_timer_callback));
			}
		}
